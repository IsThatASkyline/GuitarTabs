...
from guitar_app.application.guitar.dto import HitDTO, GetSongDTO
from guitar_app.infrastructure.db.models import HitCounterBlacklist, User, Song
...
class HitCounterRepository(BaseRepository[HitCounterBlacklist]):
    ...
    async def add_hit(self, hit_dto: GetSongDTO):
        query = update(Song).values(
            hits_count=Song.hits_count + 1
        ).where(Song.id == hit_dto.song_id)
    ...
    async def get_hit(self, hit_dto: GetSongDTO) -> HitDTO:
        subquery = select(User.id).where(User.telegram_id == hit_dto.user_id).scalar_subquery()
        query = (
            select(HitCounterBlacklist)
            .where(HitCounterBlacklist.user_id == subquery, HitCounterBlacklist.song_id == hit_dto.song_id)
        )
