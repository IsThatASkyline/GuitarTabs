...
async def create_musician_in_database(db_session_test: sessionmaker):
    async def create_musician_in_database_wrap(musician_id: int, first_name: str, last_name: str):
        async with db_session_test() as session:
            await session.execute(
                insert(Musician).values(id=musician_id, first_name=first_name, last_name=last_name)
            )
