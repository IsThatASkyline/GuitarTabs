...
from guitar_app.application.guitar.dto import CreateSongDTO, SongDTO, BandDTO, GetSongDTO
...
from tests.mocks.repo import FakeUnitOfWork, FakeSongRepo
...
async def test_get_all_songs_service():
    song_data_1 = SongDTO(id=1, title='Bigmouth Strikes Again', band=BandDTO(id=1, title='The Smiths'))
...
async def test_get_song_by_id_service():
    ...
    song_data_2 = SongDTO(id=2, title='This Charming Man', band=BandDTO(id=1, title='The Smiths'))
    repo = FakeSongRepo([song_data_1, song_data_2])
    song = await SongServices(uow=FakeUnitOfWork(song_repo=repo)).get_song_by_id(GetSongDTO(song_id=2))
    assert song.title == song_data_2.title
    with pytest.raises(SongNotExists):
        await SongServices(uow=FakeUnitOfWork(song_repo=repo)).get_song_by_id(GetSongDTO(song_id=100))
