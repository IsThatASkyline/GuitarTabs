...
async def test_create_song(
    client: AsyncClient, create_band_in_database, song_data, band_data
) -> None:
    await create_band_in_database(**band_data)
...
async def test_modulate_song(
    ...
    response = await client.post(
        f'song/modulate-song/{modulate_song_data1["song_id"]}', json=data_json1
    )
    response2 = await client.post(
        f'song/modulate-song/{modulate_song_data2["song_id"]}', json=data_json2
    )
...
async def test_add_song_to_favorite(
    ...
    response = await client.post(f'song/add-song-to-favorite/{song_data["song_id"]}', json=data_json)
...
async def test_remove_song_from_favorite(
    ...
    response = await client.post(
        f'song/remove-song-from-favorite/{song_data["song_id"]}', json=data_json
    )
...
async def test_get_user_favorite_songs(
    ...
    response = await client.get(f'song/get-user-favorite-songs/{user_data["telegram_id"]}')
