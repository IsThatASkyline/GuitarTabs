...
class GetBandById(BandUseCase):
    async def __call__(self, id_: int) -> FullBandDTO:
        if band := await self.uow.app_holder.band_repo.get_band_by_id(id_):
            ...
            return band.to_full_dto(members)
        raise BandNotExists
...
class UpdateMusicianBand(BandUseCase):
    async def __call__(self, musician_update_dto: UpdateMusicianBandDTO) -> None:
        try:
            await self.uow.app_holder.band_repo.add_musician_to_band(musician_update_dto)
            await self.uow.commit()
...
class BandServices:
    ...
    async def create_band(self, user_dto: CreateBandDTO) -> BandDTO:
    ...
    async def add_musician_to_band(self, update_musician_dto: UpdateMusicianBandDTO) -> FullBandDTO:
        ...
        # return await GetMusicianById(self.uow)(update_musician_dto.musician_id)
